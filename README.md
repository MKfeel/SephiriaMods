...
按住特殊攻击 **1.0.1** 已加入管理器更新目录：按武器机制连续释放；剑盾需同时按住普攻和特殊攻击。“放血”和“重新组装”保留原生手动操作。[源码与说明](workspace/hold-special/README.md)。编译、21 项生命周期测试及接口检查通过，游戏内与联机待验收。

IP 直连 **0.4.0 本地维护版** 已发布：增加阶段超时与取消、就绪判断、认证与角色进场检查，以及切换房间前的旧连接清理。管理器可直接安装。[源码与说明](workspace/direct-connect/README.md)。构建及 10 项离线回归通过，双机游戏验收待完成。

隐藏房间提示 **0.7.3** 已发布：移除定时全局扫描，改为事件驱动登记入口；缓存入口位置，修复屏幕外标记反复启停，保留统一黄色感叹号。[源码与说明](workspace/hidden-room-hints/README.md)。48 项模拟检查通过，游戏内待验收。

背包整理 **2.5.4** 已发布，沿用原插件 ID，快捷键 **F8**。加入 max/↑/-/↓ 标记、日志收藏同步、分层整理与组合重排，取消首次固定等待，补齐克里顿的印章优先级。[源码与说明](workspace/backpack-organizer-own/README.md)。游戏内与联机待验收。

天赋效果修改 **0.2.1** 已发布：生存20点每10最大生命值增加1%伤害放大；智慧10点迷你Boss奖励1个骰子、Boss奖励2个。已移除旧神器独特合并效果，修复重复提示。[源码与说明](workspace/talent-effects/README.md)。25项隔离检查通过，游戏内待验收。
...
**当前安装状态请看 [Mod 清单（2026-09-22）](MODS.md)**：共 14 个模组，仓库默认启用，包含根之进 0.1.5、SPMod 2.5.3 与锻体 2.8.24。背包整理为 2.5.4、隐藏房间提示为 0.7.3。本次新增按住特殊攻击 1.0.1，其余条目沿用此前核对结果；历史备份与管理器更新目录可能与本机安装状态不同。
