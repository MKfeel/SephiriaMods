...
namespace SephiriaBackpackOrganizer
{
    ...
    public class InventorySorter
    {
        ...
        public void Poll()
        {
            if (pendingSearch == null || !pendingSearch.IsCompleted)
            {
                return;
            }
            ...
            try
            {
                SearchOutcome outcome = completed.GetAwaiter().GetResult();
                FinishSortEnhanced(state, outcome);
            }
            catch (Exception ex)
            {
        ...
        private bool StartSortEnhanced(GridInventory inv)
        {
            ...
            {
                List<Slot> layout = ComputeBestLayout(inv, original, ctx,
                    out double beforeScore, out double bestScore);
        ...
        private void FinishSortEnhanced(PendingEnhancedSort state, SearchOutcome outcome)
        {
            ...
            SearchContext ctx = state.ctx;
            float beforeGame = state.beforeGameScore;
            List<Slot> finalLayout = outcome.layout;
            double beforeScore = outcome.beforeScore;
            double bestScore = outcome.bestScore;

            // 搜索期间游戏继续运行；背包只要发生过位置、旋转或物品集合变化，就丢弃旧结果。
            List<Slot> currentLayout = CaptureState(inv);
            if (!NetworkServer.active || !VerifyInventorySnapshot(inv, original) ||
                !LayoutsEquivalent(currentLayout, original))
            {
                state.stopwatch.Stop();
                Plugin.Log.LogWarning($"后台搜索完成（{state.stopwatch.ElapsedMilliseconds}ms），但背包期间已变化，结果已丢弃。");
                Notify("整理期间背包发生变化，本次结果已取消");
                return;
            }
            ...
            List<Slot> appliedLayout = CaptureState(inv);
            float finalGameScore = SafeScore(inv);
            bool layoutApplied = LayoutsEquivalent(appliedLayout, finalLayout);
            double finalOffline = EvaluateLayout(ctx, appliedLayout);
            bool gameScoreWorse = !float.IsNaN(beforeGame) && !float.IsNaN(finalGameScore) &&
                                  finalGameScore < beforeGame - 0.5f;
            bool rolledBack = false;

            // 离线搜索模型或游戏交换接口一旦与实际结果不一致，优先保证玩家已有布局不受损。
            if (!layoutApplied || gameScoreWorse)
            {
                string reason = !layoutApplied
                    ? "应用后的物品/旋转布局与搜索目标不一致"
                    : $"游戏评分下降 {beforeGame:F0} -> {finalGameScore:F0}";
                Plugin.Log.LogWarning($"整理结果安全校验失败（{reason}），正在恢复整理前布局。");
                ApplyByMoves(inv, appliedLayout, original);
                appliedLayout = CaptureState(inv);
                finalGameScore = SafeScore(inv);
                finalOffline = EvaluateLayout(ctx, appliedLayout);
                rolledBack = true;

                if (!LayoutsEquivalent(appliedLayout, original))
                {
                    Plugin.Log.LogError("恢复后布局仍与整理前快照不一致，请立即检查背包并保留日志。");
                }
            }

            if (plugin.VerboseDiagnostics.Value)
            {
                LogLayoutGrid(ctx, appliedLayout, rolledBack ? "回滚" : "整理");
                LogLayoutAnalysis(ctx, appliedLayout, rolledBack ? "回滚" : "整理");
            }

            state.stopwatch.Stop();
            Plugin.Log.LogInfo(
                $"增强整理完成（后台总耗时 {state.stopwatch.ElapsedMilliseconds}ms）：离线评分 {beforeScore:F0} -> {finalOffline:F0}" +
                $"（搜索最优 {bestScore:F0}）；游戏评分 {beforeGame:F0} -> {finalGameScore:F0}" +
                $"；搜索 {ctx.annealEvaluations} 候选/启动 {ctx.annealStartsCompleted}/{ctx.annealStarts}" +
                $"；落地校验 {(layoutApplied ? "一致" : "不一致")}{(rolledBack ? "/已回滚" : "")}" +
                $"；布局 {ctx.items.Count} 件（石板{ctx.steles.Count} 护符{ctx.charms.Count}" +
                $" 负担{ctx.burdens.Count} 其他{ctx.others.Count}）");
            // 回滚原因已经完整写入日志。对玩家统一使用原版风格的完成提示，
            // 不暴露“评分”“结果不优”等内部诊断概念。
            Notify("整理完毕");
        }
        ...
        private void SortClient(GridInventory inv)
        {
            ...
            Plugin.Log.LogInfo(
                ...
                $"离线评分 {beforeScore:F0} -> {bestScore:F0}；搜索 {ctx.annealEvaluations} 候选/" +
                $"启动 {ctx.annealStartsCompleted}/{ctx.annealStarts}；布局 {ctx.items.Count} 件" +
