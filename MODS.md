...
核对日期：2026-09-13。依据本机游戏插件元数据、安装路径和配置整理。

当前共 **10 个模组／管理工具：9 个 BepInEx 插件、1 个 AddOn**。9 个插件在磁盘上允许加载，SPMod 已禁用。“启用”表示下次启动允许加载，不代表已完成游戏内验收。
...
| 天赋效果修改 | 0.2.0 | 启用 | 生存20：每10最大生命值增加1%伤害放大，变化时立即更新；智慧10：小Boss1个骰子、大Boss2个，保留其他效果 | `SephiriaTalentEffects/SephiriaTalentEffects.dll` |
...
| 模组 | 版本 | 状态 | 当前安装路径 |
| --- | --- | --- | --- |
| SPMod（Star's SephiriaMod Mod） | 2.4.7 | **禁用** | 游戏目录下 `AddOns_Disabled/SPMod/` |
...
**本次仅同步天赋效果修改 0.2.0 的源码和安装文件，并增补 manifest.json；其他 installed/workspace 文件仍是历史备份。日常安装和更新以 catalog/mods.json 为准。** 其中可能保留已移除的插件、旧版本 DLL，以及旧的 SPMod 启用路径，不能把这些文件视为与本表一致的最新安装包。
