...
更新日期：2026-09-22。本次新增按住特殊攻击 1.0.1；其余条目沿用 2026-09-21 核对结果。

当前共 **14 个模组／管理工具：11 个 BepInEx 插件、3 个 AddOn**。仓库统一按默认启用方式提供，不区分本机启用状态；不代表已完成游戏内验收。
...
- 2026-09-22：发布按住特殊攻击 1.0.1，源码、DLL、管理器标准包与索引同步；放血／重新组装保留原生手动操作，剑盾保留单键防御。游戏内待验收。
...
- 2026-09-22：隐藏房间提示 **0.7.3** 发布。事件驱动替代定时全局扫描；源码、DLL 和管理器包同步。48 项模拟回归及生命周期契约检查通过，游戏内与联机待验收。
